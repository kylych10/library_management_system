...
export default function AdminGenresPage() {
  ...
  const getRootGenres = () => {
    return (genres || []).filter((g) => !g.parentGenreId);
  };

  const getChildGenres = (parentGenreId) => {
    return (genres || []).filter((g) => g.parentGenreId === parentGenreId);
  };
  ...
  const columns = [
    {
      ...
      renderCell: (row) => (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography
                variant="body2"
                sx={{ fontWeight: row.parentGenreId ? 400 : 600 }}
            >
              {row.name}
            </Typography>
          </Box>
    ...
    },
    {
      ...
      renderCell: (row) =>
          row.parentGenreId ? (
              <Chip
                  label={row.parentGenreName || genres?.find((g) => g.id === row.parentGenreId)?.name || "N/A"}
                  size="small"
              />
          ) : (
