...
export default function AdminUsersPage() {
  ...
  const handleUpgradeToAdmin = async (user) => {
    if (
      ...
      try {
        await dispatch(updateUserRole({ userId: user.id, role: 'ROLE_ADMIN' })).unwrap();
        loadUsers();
      } catch (error) {
        console.error('Error upgrading user:', error);
        alert('Failed to upgrade user: ' + error);
      }
    }
  };
  ...
  const handleToggleActive = async (user) => {
    ...
    if (
      ...
      try {
        await dispatch(toggleUserVerification(user.id)).unwrap();
        loadUsers();
      } catch (error) {
