...
const BookForm = ({
    ...
    const handleSubmit = async () => {
        try {
            ...
            // and ensure correct types for numeric/date fields
            const cleanedData = {
                isbn: formData.isbn || null,
                title: formData.title || null,
                author: formData.author || null,
                genreId: formData.genreId === "" ? null : Number(formData.genreId),
                publisher: formData.publisher || null,
                publicationDate: formData.publicationDate || null,
                language: formData.language || null,
                pages: formData.pages === "" || formData.pages === null ? null : Number(formData.pages),
            ...
            if (editingBook) {
                await dispatch(
                    updateBook({ id: editingBook.id, bookData: cleanedData })
                ).unwrap();
            } else {
                await dispatch(createBook(cleanedData)).unwrap();
            }
            handleCloseDialog();
        } catch (error) {
            ...
            if (typeof error === "string") {
                ...
                // Validation errors map from backend
                const messages = Object.values(error.errors).join(", ");
                setFormErrors(messages);
            } else if (error?.message) {
                setFormErrors(error.message);
            } else {
                setFormErrors("Failed to save book. Please check all fields.");
            }
        }
    };
    ...
    return (
        ...
        >
            ...
            <DialogContent dividers>
                {formErrors && (
                    <Alert severity="error" sx={{ mb: 2 }} onClose={() => setFormErrors(null)}>
                        {formErrors}
                    </Alert>
                )}
